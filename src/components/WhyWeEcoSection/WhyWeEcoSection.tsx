...
export const WhyWeEcoSection: React.FC<Props> = ({
  isAboutSection,
  isReversed,
  classNameProps,
  sectionImg,
    ...
    <section className="container mx-auto px-4 lg:px-10">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className={`lg:col-span-4 ${isReversed ? 'lg:order-2' : ''}`}>
          <div className={`${classNameProps} rounded-3xl overflow-hidden shadow-xl h-full`}>
        ...
        <div className={`lg:col-span-8 ${isReversed ? 'lg:order-1' : ''}`}>
          <div
            className={`${classNameProps} ${!isAboutSection ? 'bg-[url(/src/assets/images/why-eco/why-we-eco-background.png)] bg-cover' : ''} text-white rounded-3xl overflow-hidden shadow-xl h-full`}
          >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 items-center">
                ...
                <div className="space-y-4 text-white">
                  {paragraphs.map((paragraph, index) => (
                    <p className="text-sm xl:text-xl leading-tight" key={index}>
                      {paragraph}
                    </p>
                  ))}
                  {!isAboutSection && (
